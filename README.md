...
| Štruktúrovanie zápisu | **Claude API** (`claude-opus-5`, oficiálne PHP SDK `anthropic-ai/sdk`) – predvolené | Najlepšie rozumie dlhému kontextu (1M tokenov – aj viachodinová porada v jednom volaní), výborná slovenčina, **structured outputs** (JSON podľa schémy) – zápis, úlohy, rozhodnutia aj odhad mien rečníkov prídu v garantovanom formáte. |
| Štruktúrovanie zápisu (alternatíva) | **Google Gemini API** (`gemini-3.8-flash`, prípadne `gemini-3.1-pro-preview`) | Lacnejšia alternatíva s rovnakou JSON schémou výstupu (`responseSchema`). Prepína sa v `.env`: `AI_PROVIDER=gemini` + `GEMINI_API_KEY`. Oba poskytovatelia používajú ten istý prompt (`src/Services/Analysis/AnalysisPrompt.php`), takže výstup má rovnakú štruktúru. |
